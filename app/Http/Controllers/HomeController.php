...
class HomeController extends Controller
{
    ...
    public function totalMetrics($year)
    {
    $total_rent = DB::table('expenses')
                    ->select(DB::raw('SUM(price) as rent'))
                    ->where('category_id', 1)
    ...
    $total_earnings = DB::table('earnings')
                    ->select(DB::raw('SUM(pay) as pay'))
                    ->whereYear('start_pay', $year)
                    ->where('user_id', Auth::id())
                    ->first();

    $total_expenses = DB::table('expenses')
                    ->select(DB::raw('SUM(price) as expenses'))
                    ->whereYear('date_of_pay', $year)
                    ->where('user_id', Auth::id())
                    ->first();
    ...
    return response()->json([
        'total_rent' => '$' . number_format($total_rent->rent, 2),
        'total_earnings' => '$' . number_format($total_earnings->pay, 2),
        'total_expenses' => '$' . number_format($total_expenses->expenses, 2),
        'annual_earnings_labels' => $annual_earnings_labels,
        'annual_earnings_values' => $annual_earnings_values,
        'annual_exp_allocation_labels' => $annual_exp_allocation_labels,
        'annual_exp_allocation_data' => $annual_exp_allocation_data,
        'bar_chart_array' => $bar_chart_array,
        'total_exp_rankings' => $total_exp_rankings,
